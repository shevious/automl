...
flags.DEFINE_string('model_name', 'efficientdet-d4', 'Model.')
...
#flags.DEFINE_string('runmode', 'saved_model', 'Run mode: {freeze, bm, dry}')
...
flags.DEFINE_bool('delete_logdir', False, 'Whether to delete logdir.')
...
flags.DEFINE_string('ckpt_path', 'efficientdet-d4-eco', 'checkpoint dir used for eval.')
flags.DEFINE_string('export_ckpt', None, 'Path for exporting new models.')

flags.DEFINE_string(
    'hparams', 'eco_config.yaml', 'Comma separated k=v pairs of hyperparameters or a module'
...
flags.DEFINE_float('min_score_thresh', 0.01, 'Score threshold to show box.')
...
flags.DEFINE_string('saved_model_dir', 'efficientdet-d4-eco-saved',
                    'Folder path for saved model.')
...
class ModelInspector(object):
  ...
  def saved_model_inference(self, image_path_pattern, output_dir, **kwargs):
    ...
    image_path_pattern = os.path.join(image_path_pattern, '*')
    ...
    for i in range(num_batches):
      ...
      for j in range(size_before_pad):
        ...
        file_name = os.path.basename(all_files[i])
        print(file_name)
        file_id = os.path.splitext(file_name)[0]
        json_name = file_id + '.json'
...
def main(_):
  ...
  #image_files = glob(os.path.join(sys.argv[1], '*'))
  ...
  #FLAGS.input_image = image_directory
  inspector.run_model(
      FLAGS.runmode,
      #input_image=FLAGS.input_image,
      input_image=image_directory,
